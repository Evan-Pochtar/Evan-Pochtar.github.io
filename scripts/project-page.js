...
function createCategoryButtons() {
  ...
  Object.keys(categories).forEach((category) => {
    ...
    button.onclick = (event) =>
      handleCategoryClick(category, buttonWrapper, event);
...
function filterProjects() {
  ...
  projects.forEach((project, index) => {
    const skills = Array.from(project.querySelectorAll(".skill-item img")).map(
      (img) => img.name,
    );
    const projectDate =
      project.querySelector(".project-date")?.textContent || "";
    const matchesTags =
      selectedTags.size === 0 ||
      Array.from(selectedTags).some((tag) => skills.includes(tag));
